
<!DOCTYPE html>
<html lang="pt-br">
<head>

    <style>
    .centralizado{
        text-align: center;
    }

    .destaque{
        color:black;
        font-weight:bolder;
        font-weight:900;
    }

    .margem{
        margin-left:20px;
    }

    .nao-responder{
        color:gray;
        font-weight: bolder;
    }

    th, td {
        border: 1px solid black;
        padding: 5px;
    }

   </style>

    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- <link rel="stylesheet" type="text/css" href="./estilos.css">  não deu --> 
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>MODELO - AUTORIZAÇÃO PARA CONTRATAÇÃO - %ORIGEM_MATRICULA%</title>

</head>


<body style='font-family: sans-serif; padding: 20px'>

<!-- <h4>Para Conferir e Enviar E-mail</h4>
<ul>
    <li>Contrato: %CONTRATO_BEM%</li>
    <li>Nome Agência: %NOME_AGENCIA%</li>
    <li>Código Agência: %CODIGO_AGENCIA%</li>
    <li>Nome Proponente: %NOME_PROPONENTE%</li>
    <li>E-mail Proponente: %EMAIL_PROPONENTE%</li>
    <li>Nome Corretor: %NOME_CORRETOR%</li>
    <li>E-mail Corretor: %EMAIL_CORRETOR%</li>
    <li>Endereço Imóvel: %ENDERECO_IMOVEL%</li>
    <li>MO Utilizado: %MO_UTILIZADO%</li>
    <li>Edital Leião: %EDITAL_LEILAO%</li>
    <li>Manual Utilizado: %MN_UTILIZADO%</li>
    <li>Origem Matricula: %ORIGEM_MATRICULA%</li>
</ul> -->

<p class="nao-responder">MENSAGEM AUTOMÁTICA. FAVOR NÃO RESPONDER.</p>

<p>À</p>

<p>Agência %NOME_AGENCIA%</p>

<p>A/C Setor de Habitação</p>

<br>
    <p>Senhor (a) Gerente,</p>

    <b class="centralizado">AUTORIZAÇÃO PARA CONTRATAÇÃO À VISTA - VENDA DIRETA SEM AÇÃO JUDICIAL- %ORIGEM_MATRICULA%</b><br><br><br>

    <b>1   DA AUTORIZAÇÃO:</b>
    <br>    

    <p>
        1.1  Autorizamos a contratação de imóvel adjudicado oriundo do Edital %EDITAL_LEILAO%, conforme %MN_UTILIZADO% :
    </p>

    <table>
        <thead>
            <tr>
                <th class="centralizado"><b>Número do Contrato Anterior</b></th>
                <th class="centralizado"><b>Endereço Completo do Imóvel</b></th>
                <th class="centralizado"><b>Nome do Proponente</b></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td class="centralizado">%CONTRATO_BEM%</td>
                <td class="centralizado">%ENDERECO_IMOVEL%</td>
                <td class="centralizado">%NOME_PROPONENTE%</td>
            </tr>
        </tbody>
    </table>

    <p>
        1.2  Os documentos referentes ao imóvel e aquisição encontram-se disponíveis para consulta, exclusivamente 
        pela agência Caixa, no link: \\sp7257sr001\PUBLIC\EstoqueImoveis\%CONTRATO_BEM%
    </p>

    <b>2    DO PAGAMENTO:</b>
    <br>

    <p>
        2.1   Por gentileza, solicitamos entrar em contato com o cliente através dos telefones constantes na proposta;
    </p>

    <p>
        2.2   Alertamos para o prazo máximo de 05 (cinco) dias úteis contado da data da divulgação do resultado homologado 
	para efetuar o pagamento dos recursos próprios. Tal prazo poderá ser prorrogado a critério da Caixa;</b>;
    </p>

    <p>
        2.3   É de responsabilidade do proponente vencedor a obtenção da guia de pagamento através do site www.caixa.gov.br\ximoveis, 
	acessível na área restrita do proponente vencedor a partir da divulgação do resultado e sua autenticação dentro do prazo de vencimento;
    </p>

    <b>3    DA CONTRATAÇÃO:</b>
    <br>

    <p>
        3.1   3.1  Cabe à agência responsável pela contratação fornecer o modelo de Escritura Pública adequado a cada caso e orientar o cliente 
	durante o processo de transferência do imóvel,
    </p>
    
    <p>
        3.2  As Certidões Negativas da CAIXA (Receita Federal ou INSS), podem ser solicitadas às SRs de vinculação.
    </p>

        <b>4    DO REGISTRO:</b>
    <br>

    <p>
        4.1   Cabe ao comprador tomar todas as providências necessárias para o registro do Contrato de financiamento habitacional 
        ou Contrato de Aquisição à Vista com FGTS junto ao Oficial de Registro de Imóveis competente, conforme %MO_UTILIZADO%;
    </p>

    <p>
        4.2   Após isso, deverá apresentar junto à agência Caixa a matrícula atualizada com o registro do contrato e a 
        atualização dos dados cadastrais junto à Prefeitura Municipal, conforme determinação constante no Edital;  
    </p>

    <p>
        4.3   As Certidões Negativas da CAIXA (Receita Federal ou INSS), podem ser solicitadas às SRs de vinculação;  
    </p>

    <b>5    DOS DÉBITOS DO IMÓVEL:</b>
    <br>

    <p>
        5.1   Caso haja débitos de IPTU e/ou CONDOMÍNIO pendentes orientamos que estes deverão ser enviados pelo proponente 
	para a caixa postal giliesp07@caixa.gov.br;
    </p>
    
    <p>
        5.2   Em relação, às contas de energia elétrica e água pendentes, orientar o comprador a levar o contrato à Concessionária 
        de Energia e Água, para que seja feita a religação em seu nome, os débitos anteriores ficarão por conta do antigo morador.
    </p>

    <b>6    DA IMPOSSIBILIDADE DA CONTRATAÇÃO:</b>

    <p>
        6.1   Caso haja qualquer impossibilidade para prosseguimento da contratação, como por exemplo, a desistência do proponente, 
        comunicar IMEDIATAMENTE a esta GILIE/SP para adoção das providências cabíveis.
    </p>
	7.     DA UTILIZAÇÃO DO CORRETOR CREDENCIADO CAIXA
    </p>
     <p>
	7.1   Nos casos de assessoramento por corretor credenciado CAIXA, a remuneração pelos serviços será paga exclusivamente pela CAIXA;
    </p>
     <p>
	7.2   Ressaltamos que compete ao corretor, assessorar o proponente quando da apresentação de proposta, não sendo aceita indicação de corretores 
	após a data de vencimento da guia referente ao sinal;
    </p>
     <p>
	7.3   Esclarecemos que o corretor credenciado deve atuar desde a apresentação da proposta, na obtenção de crédito de financiamento junto à CAIXA,
	nos registros da operação perante o cartório de registro de imóveis e prefeitura na intermediação de pagamento de débitos de responsabilidade da CAIXA incidentes sobre o imóvel até e auxílio na desocupação do imóvel, se for o caso.
     </p>

    <p><br>
    <b>8.</b> Sem mais, agradecemos e colocamo-nos a disposição para dirimir quaisquer dúvidas pelo e-mail: giliesp01@caixa.gov.br, 
    ou consulte o site %QUADRO_EMPREGADOS_POR_ATIVIDADE% e contate um colega da equipe de contratação.
    </p>
    
<p class="destaque">
Atenciosamente,
<br>
Equipe GILIE/SP
</P>


</body>
</html>