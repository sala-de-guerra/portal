
<!DOCTYPE html>
<html lang="pt-br">
<head>

    <style>
    .centralizado{
        text-align: center;
    }

    .destaque{
        color:black;
        font-weight:bolder;
        font-weight:900;
    }

    .margem{
        margin-left:20px;
    }

    .nao-responder{
        color:gray;
        font-weight: bolder;
    }

    p {
        margin: 5px;
    }

    .pl-20px{
        padding-left: 20px;
    }

    .pl-40px{
        padding-left: 40px;
    }

    td {
        text-align: justify;
        vertical-align: text-top;
    }

   </style>

    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- <link rel="stylesheet" type="text/css" href="./estilos.css">  não deu --> 
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>SOLICITAÇÃO DE DOCUMENTOS REEMBOLSÁVEIS DE DISTRATO - PROTOCOLO %ID_DISTRATO%</title>

</head>

<body style='font-family: sans-serif; padding: 20px'>

<!-- DE GILIESP01@CAIXA.GOV.BR -->
<!-- PARA  %EMAIL_PROPONENTE% %EMAIL_CORRETOR% -->
<!-- CC %EMAIL_AGENCIA% -->
<!-- CCO GILIESP01@CAIXA.GOV.BR; %USUARIO_SESSAO% -->
<!-- ASSUNTO Solicitação de documentos para processo de distrato - Comprador %NOME_PROPONENTE% - CHB %CONTRATO_BEM%-->

    <p>Prezados(as) Senhores(as),</p>

    <br>
    <p>C/C Agência %NOME_AGENCIA%</p>

    <br>
    <table>
        <thead></thead>
        <tbody>
            <tr>
                <td>
                    <b class="centralizado">Assunto:</b>
                </td>
                <td>
                    <b class="pl-40px">Distrato de Imóvel comprado na modalidade %MODALIDADE_VENDA% - Nº do Bem: %CONTRATO_BEM%</b>
                </td>
            </tr>
            <tr>
                <td>
                    <b class="centralizado">Arrematante:</b>
                </td>
                <td>
                    <b class="pl-40px">%NOME_PROPONENTE_DISTRATO% - CPF / CNPJ: %CPF_CNPJ_PROPONENTE%</b>
                </td>
            </tr>
            <tr>
                <td>
                    <b class="centralizado">Motivo:</b>
                </td>
                <td>
                    <b class="pl-40px">%MOTIVO_DISTRATO%</b>
                </td>
            </tr>
        </tbody>
    </table>

    <br>

    <table>
        <thead></thead>
        <tbody>
            <tr>
                <td>
                    1
                </td>
                <td class="pl-20px">
                    Considerando a necessidade de distrato pelo motivo citado acima, para o imóvel arrematado 
                    pelo senhores, a venda será cancelada e os valores pagos serão devolvidos.
                </td>
            </tr>
            <tr>
                <td>
                    2
                </td>
                <td class="pl-20px">
                    A devolução dos valores e assinatura do documento de cancelamento de venda, se houver, 
                    serão realizados através da Agência de contratação, em até 15 dias úteis após a recepção dos comprovantes 
                    de pagamento e autorização da GILIE.
                </td>
            </tr>
            <tr>
                <td>
                    3
                </td>
                <td class="pl-20px">
                    Os valores passíveis de devolução pela Caixa são os que seguem:
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>valores pagos a título de recursos próprios tais como sinal, prestação de 
                    financiamento e tarifa de concessão de financiamento;</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>comissão do leiloeiro;</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>valor das dívidas propter rem pagas pelo adquirente e devidamente comprovadas (Condomínio, IPTU);</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>ITBI;</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>lavratura de escritura;</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>registro em cartório;</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>honorários Advocatícios</b>
                </td>
            </tr>
            <tr>
                <td>
                    4
                </td>
                <td class="pl-20px">
                    Solicitamos que seja encaminhado também:
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>Cópia simples do Contrato assinado com a Caixa Econômica Federal ou Escritura Pública 
                    de Compra e Venda, caso tenha sido assinado.</b>
                </td>
            </tr>
            <tr>
                <td class="pl-40px">
                    <b>•</b>
                </td>
                <td class="pl-20px">
                    <b>Cópia simples da matricula onde consta a averbação de venda do imóvel por parte da Caixa 
                        Econômica Federal, caso a venda tenha sido averbada.</b>
                </td>
            </tr>
            <tr>
                <td>
                    5
                </td>
                <td class="pl-20px">
                    Caso haja despesas não contempladas na relação acima, tais como despesas com desocupação 
                    ou reforma do imóvel, vossa senhoria poderá encaminhar a solicitação de devolução com 
                    os comprovantes de pagamento, para análise.
                </td>
            </tr>
            <tr>
                <td>
                    6
                </td>
                <td class="pl-20px">
                    Sobre os valores autorizados para devolução será calculada atualização monetária composta de 
                    remuneração básica e juros da poupança, e será calculada da data do pagamento até a data da devolução.
                </td>
            </tr>
            <tr>
                <td>
                    7
                </td>
                <td class="pl-20px">
                    Solicitamos a gentileza de enviar os comprovantes de pagamento acima elencados por mensagem 
                    eletrônica para giliesp01@caixa.gov.br , <b>cujos anexos não poderão ultrapassar o tamanho de 2 Megas por 
                    mensagem</b>, se necessário poderá distribuir os anexos em mais de um e-mail, ou ainda se preferir, 
                    entregar os documentos pessoalmente nesta GILIESP, localizada na Avenida Paulista 1294 – 5º Andar, 
                    Edifício Eluma, 01310-915 – São Paulo – SP, horário de atendimento das 10:00 às 16:00 horas.
                </td>
            </tr>
            <tr>
                <td>
                    8
                </td>
                <td class="pl-20px">
                    Aguardamos o envio dos documentos para iniciarmos os procedimentos para o distrato.
                </td>
            </tr>
            <tr>
                <td>
                    9
                </td>
                <td class="pl-20px">
                    Eventuais dúvidas poderão ser sanadas em resposta à presente mensagem.
                </td>
            </tr>

            <tr>
                <td>
                    10
                </td>
                <td class="pl-20px">
                    Lamentamos os transtornos gerados pela situação, e colocamo-nos à disposição.
                </td>
            </tr>
        </tbody>
    </table>

    <br>        
    <p class="destaque">
    Atenciosamente,
    <br>
    GILIE/SP | GI ALIENAR BENS MOVEIS E IMOVEIS
    </P>


</body>
</html>