
<!DOCTYPE html>
<html lang="pt-br">
<head>

    <style>
    .centralizado{
        text-align: center;
    }

    .destaque{
        color:black;
    }

    .margem{
        margin-left:20px;
    }

    .nao-responder{
        color:gray;
        font-weight: bolder;
    }

    th, td {
        border: 1px solid black;
        padding: 5px;
    }

   </style>

    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- <link rel="stylesheet" type="text/css" href="./estilos.css">  não deu --> 
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>MODELO - AUTORIZAÇÃO PARA CONTRATAÇÃO VENDA ONLINE FINANCIADO - %ORIGEM_IMOVEL%</title>

</head>


<body style='font-family: sans-serif; padding: 20px'>

<!-- <h4>Para Conferir e Enviar E-mail</h4>
<ul>
    <li>Contrato: %CONTRATO_BEM%</li>
    <li>Nome Agência: %NOME_AGENCIA%</li>
    <li>Código Agência: %CODIGO_AGENCIA%</li>
    <li>Nome Proponente: %NOME_PROPONENTE%</li>
    <li>E-mail Proponente: %EMAIL_PROPONENTE%</li>
    <li>Nome Corretor: %NOME_CORRETOR%</li>
    <li>E-mail Corretor: %EMAIL_CORRETOR%</li>
    <li>Endereço Imóvel: %ENDERECO_IMOVEL%</li>
    <li>MO Utilizado: %MO_UTILIZADO%</li>
    <li>Edital Leião: %EDITAL_LEILAO%</li>
    <li>Manual Utilizado: %MN_UTILIZADO%</li>
    <li>Origem Matricula: %ORIGEM_MATRICULA%</li>
</ul> -->

<p class="nao-responder">MENSAGEM AUTOMÁTICA. FAVOR NÃO RESPONDER.</p>

<p style="color: red;"><b>***ATENÇÃO***</b>   devido ao novo Corona vírus as agências da Caixa Econômica Federal <br>
estão com atendimento contingencial e reduzido no horário das 10h às 14h***</p>
<p style="color: red;"><b>confira a lista de agências abertas acessando:</b> <a href="www.caixa.gov.br/caixacomvoce/Paginas/default.aspx#horario">www.caixa.gov.br/caixacomvoce/Paginas/default.aspx#horario</a></p>

<p>À</p>

<p>Agência %NOME_AGENCIA%</p>

<p>C/C %NOME_PROPONENTE%</p>

<p>A/C Setor de Habitação</p>

<br>
    <p>Senhor (a) Gerente,</p>

    <b class="centralizado">AUTORIZAÇÃO PARA CONTRATAÇÃO VENDA ONLINE COM FINANCIAMENTO E/OU FGTS</b><br><br><br>

    <b>1   DA AUTORIZAÇÃO:</b>
    <br>    

    <p>
        1.1  Autorizamos a contratação <b>com Financiamento e/ou FGTS</b> do imóvel adjudicado abaixo, conforme <b>
        %MN_UTILIZADO%</b>:
    </p>

    <table>
        <thead>
            <tr>
                <th class="centralizado"><b>Número do Contrato Anterior</b></th>
                <th class="centralizado"><b>Endereço Completo do Imóvel</b></th>
                <th class="centralizado"><b>Nome do Proponente</b></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td class="centralizado">%CONTRATO_BEM%</td>
                <td class="centralizado">%ENDERECO_IMOVEL%</td>
                <td class="centralizado">%NOME_PROPONENTE%</td>
            </tr>
        </tbody>
    </table>

    <p>
        1.2  Os dados referentes ao imóvel e aquisição encontram-se disponíveis para consulta, exclusivamente 
        pela agência Caixa, no portal: <a href="https://portal.gilie.sp.caixa/">https://portal.gilie.sp.caixa/</a>;
    </p>
    
    <p>
        1.3  Os documentos referentes ao imóvel e aquisição encontram-se disponíveis para consulta, exclusivamente 
        pela agência Caixa, no link: \\arquivos.caixa\sp\SP7257FS201\PUBLICO\PUBLIC\EstoqueImoveis\%CONTRATO_BEM%
    </p>

    <b>2    DO PAGAMENTO DOS RECURSOS PRÓPRIOS:</b>
    <br>

    <p>
        2.1   O adquirente deverá comparecer na agência Caixa com a guia de pagamento (PP15) impressa através do site 
        www.caixa.gov.br\ximoveis, acessível na área restrita do proponente vencedor a partir da divulgação do resultado;
    </p>

    <p>
        2.2   Alertamos para o prazo máximo de 05 (cinco) dias úteis contado da data da divulgação do resultado homologado para 
        efetuar o pagamento dos recursos próprios. Tal prazo poderá ser prorrogado a critério da Caixa;
    </p>

    <p>
        2.3   É de responsabilidade do proponente vencedor a obtenção da guia de pagamento através do site da Caixa e sua 
        autenticação dentro do prazo de vencimento;
    </p>

    <p>
        2.4   Na data da autenticação, enviar comprovante de pagamento digitalizado para o e-mail giliesp01@caixa.gov.br.
    </p>
    <p>
        2.5 A guia de pagamento / boleto deve ser apresentada ao <b>GERENTE</b> da unidade para proceder conforme item 3
    </p>

    <b>3    DA CONTRATAÇÃO:</b>
    <br>

    <p>
        3.1   Alertamos que todos os imóveis em contratação pela GILIE/SP que contarão com financiamento habitacional 
        e/ou FGTS estão incluídos na fila: FILA - IMÓVEIS CAIXA/EMGEA - CONTRATAÇÃO;
    </p>
    
    <p>
        3.2   Após a conformidade pela CICOB/RE da documentação enviada pela GILIE/SP, a Agência deverá proceder a digitalização 
        dos documentos do cliente na estação de captura SICT2 (FILA - IMOVEIS CAIXA/EMGEA – CONTRATAÇÃO), para complementação do 
        dossiê habitacional e prosseguimento do processo de contratação;
    </p>

    <p>
        3.3   Em caso de dúvidas sobre o envio e tratamento da documentação do COMPRADOR pela CICOB/RE, 
        favor acessar o ATENDE: http://inovacao.suban.caixa/#/atende.
    </p>

    <b>4    DO REGISTRO:</b>
    <br>

    <p>
        4.1   Cabe ao comprador tomar todas as providências necessárias para o registro do Contrato de financiamento 
        habitacional ou Contrato de Aquisição à Vista com FGTS junto ao Oficial de Registro de Imóveis competente;;
    </p>

    <p>
        4.2   Após isso, deverá apresentar junto à agência Caixa a matrícula atualizada com o registro da venda e compra, 
        e a atualização dos dados cadastrais junto à Prefeitura Municipal, conforme determinação constante no Edital;  
    </p>

    <p>
        4.3   As Certidões Negativas da CAIXA (Receita Federal ou INSS), podem ser solicitadas às SRs de vinculação.  
    </p>
    <p>
        4.4   <b>Todos os dados referentes ao imóvel, proponente e contratação podem ser acessados através do:</b> <a href="https://portal.gilie.sp.caixa/">https://portal.gilie.sp.caixa/</a>;  
    </p>


    <b>5    DOS DÉBITOS DO IMÓVEL:</b>
    <br>

    <p>
        5.1   Caso haja débitos de IPTU e/ou CONDOMÍNIO pendentes orientamos que estes deverão ser enviados pelo proponente 
        para a caixa postal giliesp07@caixa.gov.br;
    </p>
    
    <p>
        5.2   Em relação, às contas de energia elétrica e água pendentes, orientar o comprador a levar o contrato à Concessionária 
        de Energia e Água, para que seja feita a religação em seu nome, os débitos anteriores ficarão por conta do antigo morador.
    </p>

    <b>6    DA UTILIZAÇÃO DO CORRETOR CREDENCIADO CAIXA</b>
    <br>

    <p>
        6.1   Nos casos de assessoramento por corretor credenciado CAIXA, a remuneração pelos serviços será paga exclusivamente 
        pela CAIXA;
    </p>

    <p>
        6.2   Ressaltamos que compete ao corretor, assessorar o proponente quando da apresentação de proposta, não sendo aceita 
        indicação de corretores após a data de vencimento da guia referente ao sinal;
    </p>

    <p>
        6.3   Esclarecemos que o corretor credenciado deve atuar desde a apresentação da proposta, na obtenção de crédito de 
        financiamento junto à CAIXA, nos registros da operação perante o cartório de registro de imóveis e prefeitura na 
        intermediação de pagamento de débitos de responsabilidade da CAIXA incidentes sobre o imóvel até e auxílio na desocupação 
        do imóvel, se for o caso.
    </p>

    <b>7    DA IMPOSSIBILIDADE DA CONTRATAÇÃO:</b>

    <p>
        7.1   Caso haja qualquer impossibilidade para prosseguimento da contratação, como por exemplo, a desistência do proponente, 
        comunicar IMEDIATAMENTE a esta GILIE/SP para adoção das providências cabíveis.
    </p>

    <p><br>
    <b>8</b> Sem mais, agradecemos e colocamo-nos a disposição para dirimir quaisquer dúvidas pelo e-mail: giliesp01@caixa.gov.br, 
    ou consulte o site %QUADRO_EMPREGADOS_POR_ATIVIDADE% e contate um colega da equipe de contratação.
    </p>
    
<p class="destaque">
Atenciosamente,
<br><br>
Equipe GILIE/SP | GI ALIENAR BENS MOVEIS E IMOVEIS<br>
<a href="https://portal.gilie.sp.caixa/">https://portal.gilie.sp.caixa/</a>;
</P>


</body>
</html>